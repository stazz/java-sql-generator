...
public abstract class ComposedBooleanExpressionImpl<ExpressionType extends ComposedBooleanExpression> extends
    ...
    protected ComposedBooleanExpressionImpl(final SQLProcessorAggregator processor,
                                            final Class<? extends ExpressionType> expressionClass) {
        super(processor, expressionClass);
    }

    @Override
    protected boolean doesEqual(final ExpressionType another) {
        final Iterator<BooleanExpression> thisIter = this.iterator();
        final Iterator<BooleanExpression> anotherIter = another.iterator();
