...
public class BooleanUtils {
    ...
    public static Boolean isEmpty(final BooleanExpression expression) {
        Boolean result = expression == EmptyPredicate.INSTANCE;
        if (!result && expression instanceof ComposedBooleanExpression) {
            for (final BooleanExpression exp : (ComposedBooleanExpression) expression) {
                result = BooleanUtils.isEmpty(exp);
                if (!result) {
                    break;
                }
            }
