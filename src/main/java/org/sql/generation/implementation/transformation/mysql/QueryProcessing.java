...
public class QueryProcessing {
    ...
    public static class MySQLQuerySpecificationProcessor extends QuerySpecificationProcessor {
        ...
        @Override
        protected void processLimitAndOffset(final SQLProcessorAggregator processor, final StringBuilder builder,
                                             Typeable<?> first, final Typeable<?> second) {
            final MySQLVendor vendor = (MySQLVendor) processor.getVendor();
            if (vendor.legacyLimit()) {
                // Just do the processing right away, because of the difference of syntax
                builder.append(SQLConstants.NEWLINE).append(QueryProcessing.MYSQL_LIMIT_PREFIX)
    ...
    public static class MySQLOffsetSpecificationProcessor extends OffsetSpecificationProcessor {
        ...
        protected String getPostfix(final SQLProcessorAggregator processor) {
            return QueryProcessing.MYSQL_OFFSET_POSTFIX;
        }
    }
    ...
    public static class MySQLLimitSpecificationProcessor extends LimitSpecificationProcessor {
        ...
        @Override
        protected String getPostfix(final SQLProcessorAggregator processor) {
            return QueryProcessing.MYSQL_LIMIT_POSTFIX;
        }
    }
}
